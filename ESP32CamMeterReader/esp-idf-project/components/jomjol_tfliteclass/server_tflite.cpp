...
esp_err_t handler_init(httpd_req_t *req)
{
    ...
    const char* resp_str = "<p style=\"font-size: 18px; font-weight: bold; font-family: monospace; color: #5347AE;\">Инициализация запущена</p>";
    httpd_resp_send(req, resp_str, strlen(resp_str));     

    doInit();

    resp_str = "<p style=\"font-size: 18px; font-weight: bold; font-family: monospace; color: #5347AE;\">Инициализация завершена</p>";
    httpd_resp_send(req, resp_str, strlen(resp_str));     
...
esp_err_t handler_doflow(httpd_req_t *req)
{
    ...
    if (flowisrunning)
    {
        const char* resp_str = "<p style=\"font-size: 18px; font-weight: bold; font-family: monospace; color: #5347AE;\">Функция \"doFlow\" уже запущена и не может быть запущена снова!</p>";
        httpd_resp_send(req, resp_str, strlen(resp_str));       
        return 2;
    }
    else
    ...
    const char* resp_str = "<p style=\"font-size: 18px; font-weight: bold; font-family: monospace; color: #5347AE;\">Функция \"doFlow\" запущена - это займёт около 60 секунд</p>";
    httpd_resp_send(req, resp_str, strlen(resp_str));  
    /* Respond with an empty chunk to signal HTTP response completion */
    httpd_resp_send_chunk(req, NULL, 0);       
...
//        printf("Query: "); printf(_query); printf("\n");
    ...
    if (_all)
    {
        ...
        zw = "<p style=\"font-size: 20px; font-weight: bold; font-family: monospace; color: #5347AE;\">Показания счётчика: " + tfliteflow.getReadoutAll(_intype) + "</p>";
        printf("ZW: %s\n", zw.c_str());
        if (zw.length() > 0)
            httpd_resp_sendstr_chunk(req, zw.c_str()); 
        httpd_resp_sendstr_chunk(req, NULL);   
    ...
    zw = "<p style=\"font-size: 20px; font-weight: bold; font-family: monospace; color: #5347AE;\">Показания счётчика: " + tfliteflow.getReadout(_rawValue, _noerror) + "</p>";
    if (zw.length() > 0)
        httpd_resp_sendstr_chunk(req, zw.c_str()); 
...
//    printf("Query: %s\n", query.c_str());
    if (query.find("full") != std::string::npos)
    {
        ...
        txt = "<p style=\"font-size: 20px; font-weight: bold; font-family: monospace; color: #5347AE;\">Выровненное изображение: <p><img src=\"/img_tmp/alg_roi.jpg\"> <p>\n";
        txt = txt + "<p style=\"font-size: 20px; font-weight: bold; font-family: monospace; color: #5347AE;\">Показания цифрового счётчика: <p> ";
        ...
        for (int i = 0; i < htmlinfodig.size(); ++i)
        {
            if (tfliteflow.GetTypeDigital() == Digital)
            {
                if (htmlinfodig[i]->val == 10)
                    zw = "<label style=\"font-size: 18px; font-weight: bold; font-family: monospace; color: #5347AE;\">NaN</label>";
                else
                    zw = "<label style=\"font-size: 18px; font-weight: bold; font-family: monospace; color: #5347AE;\">" + to_string((int)htmlinfodig[i]->val) + "</label>";
            ...
            else
            {
                ...
                zw = "<label style=\"font-size: 18px; font-weight: bold; font-family: monospace; color: #5347AE;\">" + stream.str() + "</label>";
        ...
        txt = " <p style=\"font-size: 20px; font-weight: bold; font-family: monospace; color: #5347AE;\"> Показания аналогового счётчика: <p> ";
        httpd_resp_sendstr_chunk(req, txt.c_str()); 
...
esp_err_t handler_prevalue(httpd_req_t *req)
{
    ...
    {
        zw = "Новое предвар. значение: " + tfliteflow.UpdatePrevalue(_size, _numbers, true);
    }
...
void task_autodoFlow(void *pvParameter)
{
    ...
    printf("Auto Interval: ");
    printf((to_string(auto_intervall)).c_str());
    printf(" ms\n");
    ...
    {        
        ...
        fr_delta_ms = esp_timer_get_time() / 1000;
...
void TFliteDoAutoStart()
{
    ...
    xReturned = xTaskCreate(&task_autodoFlow, "task_autodoFlow", configMINIMAL_STACK_SIZE * 45, NULL, tskIDLE_PRIORITY+1, &xHandletask_autodoFlow);
    if( xReturned != pdPASS )
    {
